...
export default function CoordinacionDocentesPage() {
  ...
  const docentes = usuariosMock.filter((u) => u.rol === "PROFESOR")
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#D9D9D9] rounded-2xl p-6">
        <h1 className="text-2xl font-bold text-[#353535]">Docentes</h1>
        <p className="text-[#6B7280] mt-1">Personal docente activo en la carrera</p>
      </div>
        ...
        {carrerasMock.map((c) => (
          <Card key={c.id} className="border-[#D9D9D9]">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-[#6B7280] mt-0.5">{c.siglas}</p>
            </CardContent>
          </Card>
        ))}
        <Card className="border-[#D9D9D9]">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-[#3C6E71]">{docentes.length}</p>
      ...
      <Card className="border-[#D9D9D9]">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2 text-[#353535]">
            <Users className="w-4 h-4 text-[#3C6E71]" />
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar docente..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 border-[#D9D9D9]" />
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((d) => {
              ...
              return (
                <div key={d.id} className="border border-[#D9D9D9] rounded-xl p-4 hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-full bg-[#3C6E71]/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-[#3C6E71] text-sm font-bold">
                        {d.nombres.charAt(0)}{d.apellidos.charAt(0)}
                      </span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-[#353535] truncate">{d.nombres} {d.apellidos}</p>
                      {carrera && (
                        <Badge className="text-xs border-0 bg-[#E0EEEF] text-[#3C6E71] mt-0.5">{carrera.siglas}</Badge>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 mt-3 pt-3 border-t border-[#F5F5F5]">
                    <div className="flex-1 text-center">
                      <div className="flex items-center justify-center gap-1 text-[#3C6E71]">
                        <BookOpen className="w-3.5 h-3.5" />
                        <span className="text-sm font-bold">{materias.length}</span>
                      </div>
                      <p className="text-xs text-[#9CA3AF]">Materias</p>
                    </div>
                    <div className="flex-1 text-center">
                      <div className="flex items-center justify-center gap-1 text-[#536493]">
                        <FileText className="w-3.5 h-3.5" />
                        <span className="text-sm font-bold">{informes.length}</span>
                      </div>
                      <p className="text-xs text-[#9CA3AF]">Informes</p>
                    </div>
                    {pendientes > 0 && (
                      <div className="flex-1 text-center">
          ...
          {filtered.length === 0 && (
            <p className="py-8 text-center text-sm text-[#9CA3AF]">No se encontraron docentes.</p>
          )}
        </CardContent>
      </Card>
    </div>
